...
var logger = function(obj) {
  switch (obj.type) {
    case 'error':
      ...
      console.error('mimeType:', obj.mimeType);
...
var compile = function(fileName, assets, S3, options, method, type, timestamp, callback) {
  ...
  return function(err, results) {
    ...
    var expires = new Date(new Date().getTime() + (31556926 * 1000));
    ...
    switch (method) {
      case 'uglify':
        ...
        try {
          ...
          if (final_code && final_code.error) {
            ...
            return finishUpload(final_code.error);
            break;
          }
          ...
          uploadOptions.Key = fileName;
          uploadOptions.Body = buffer;
          logger({type: 'log', msg: 'Starting to upload script to S3', path: assets, file: fileName});
      ...
      case 'minify':
        ...
        zlib.gzip(final_code.join("\n"), function(err, buffer) {
          ...
          uploadOptions.Key = fileName;
          uploadOptions.Body = buffer;
          logger({type: 'log', msg: 'Starting to upload stylesheet to S3', path: assets, file: fileName});
          S3.upload(uploadOptions, function (err, data) {
            if (err){
      ...
      case 'optipng':
        ...
        optipng.stderr.on('data', function(data) {
          logger({type: 'warn', msg: 'Failed on optipng', path: assets, file: fileName, warn: data});
        });
        optipng.on('exit', function(code) {
          ...
          fs.readFile(img, function(err, data) {
            ...
            zlib.gzip(data, function(err, buffer) {
              ...
              uploadOptions.Key = fileName;
              uploadOptions.Body = buffer;
              logger({type: 'log', msg: 'Starting to upload image to S3', path: assets, file: fileName});
              S3.upload(uploadOptions, function (err, data) {
                if (err){
      ...
      case 'font':
        ...
        fs.readFile(image, function(err, data) {
          if (err) {
            logger({type: 'error', msg: 'Failed to read font to S3', path: assets, file: fileName, err: err});
            return finishUpload();
          }
          zlib.gzip(data, function(err, buffer) {
            if (err) {
              logger({type: 'error', msg: 'Failed to compress font', path: assets, file: fileName, S3: S3, err: err});
              return finishUpload();
            }

            uploadOptions.Key = fileName;
            uploadOptions.Body = buffer;
            logger({type: 'log', msg: 'Starting to upload font to S3', path: assets, file: fileName});
            S3.upload(uploadOptions, function (err, data) {
              if (err) {
                logger({type: 'error', msg: 'Unsuccessful upload of font to S3', path: assets, file: fileName, S3: S3, err: err});
                return finishUpload();
              }

              logger({type: 'info', msg: 'Successfully uploaded font to S3', path: assets, file: fileName, S3: S3});
              //Hack to preserve original timestamp for view helper
              fs.utimesSync(image, new Date(timestamp), new Date(timestamp));
              return finishUpload();
            });
...
var checkArrayIfModified = function(assets, fileName, S3, options, timestamp, type, callback) {
  ...
  return function(response) {
    if (response.httpResponse && response.httpResponse.statusCode === 200 && response.data && timestamp <= Date.parse(response.data.LastModified)) {
      logger({type: 'info', msg: 'Faile not modified and is already stored on S3', path: assets, file: fileName, S3: S3});
      return finishUpload();
    }
    else if(response.statusCode != 404 && response.code != 'NotFound' && (!response.httpResponse || response.httpResponse.statusCode != 200)){
      logger({type: 'error', msg: 'Failed to check multiple files heads on S3', path: assets, file: fileName, S3: S3, err: response});
      return finishUpload();
    }
    else {
      ...
      switch (type) {
        ...
        case 'text/css':
          ...
          return;
        default:
          logger({type: 'error', msg: 'Unsupported mime type in array: "' + type + '"', path: assets, file: fileName, mimeType: type});
          return;
      }
    }
  };
...
var checkStringIfModified = function(assets, fileName, S3, options, timestamp, callback) {
  ...
  return function(response) {
    if (response.httpResponse && response.httpResponse.statusCode === 200 && response.data && timestamp <= Date.parse(response.data.LastModified)) {
      ...
      logger({type: 'error', msg: 'Failed to check file head on S3', path: assets, file: fileName, S3: S3, err: response});
      return finishUpload();
    }
    else {
      ...
      switch (type) {
        case 'application/javascript':
        case 'text/javascript':
          return readUtf8(assets, compile(fileName, assets, S3, options, 'uglify', type, null, finishUpload));
        case 'text/css':
          return readUtf8(assets, compile(fileName, assets, S3, options, 'minify', type, null, finishUpload));
        case 'image/gif':
        case 'image/x-icon':
        case 'image/svg+xml':
          return compile(fileName, assets, S3, options, 'image', type, timestamp, finishUpload)(null, null);
        case 'image/png':
          return compile(fileName, assets, S3, options, 'optipng', type, timestamp, finishUpload)(null, null);
        case 'image/jpg':
        case 'image/jpeg':
        case 'image/pjpeg':
          return compile(fileName, assets, S3, options, 'jpegtran', type, timestamp, finishUpload)(null, null);
        case 'image/x-icon':
        case 'image/vnd.microsoft.icon':
          return compile(fileName, assets, S3, options, 'image', type, timestamp, finishUpload)(null, null);
        default:
  ...
  async.map(results, function(result, iter) {
    ...
    if (assets instanceof Array) {
      ...
      for (var k = 0; k < assets.length; k += 1) {
        if (type === ''){
          type = mime.lookup(assets[k]);
        }
        else if (mime.lookup(assets[k]) !== type){
          logger({type: 'error', msg: 'Mime type in array do not match', path: assets, file: fileName});
        }

        assets[k] = path.join(options.publicDir, assets[k]);

        try {
          fs.accessSync(assets[k]);
        } catch (err) {
          logger({type: 'error', msg: 'File ' + assets[k] + 'does not exist', path: assets, file: fileName, S3: awsS3});
        } finally {
      ...
      awsS3.headObject({Bucket: options.bucket, Key: fileName})
      .on('success', checkArrayIfModified(assets, fileName, awsS3, options, timestamp, type, iter))
      .on('error', checkArrayIfModified(assets, fileName, awsS3, options, timestamp, type, iter))
      .send();
    }
    else {
      // Set the file name
      ...
      try {
        ...
        logger({type: 'warn', msg: 'File does not exist', path: assets, file: fileName, S3: awsS3});
      } finally {
        timestamp = fs.statSync(assets).mtime.getTime();

        awsS3.headObject({Bucket: options.bucket, Key: fileName})
        .on('success', checkStringIfModified(assets, fileName, awsS3, options, timestamp, iter))
        .on('error', checkStringIfModified(assets, fileName, awsS3, options, timestamp, iter))
        .send();
      }
    }
